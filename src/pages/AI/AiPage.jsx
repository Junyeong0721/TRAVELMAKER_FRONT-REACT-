...
const AIPage = () => {
  ...
  // ✅ [수정 1] Fallback 데이터에도 'category' 필드 추가 (백엔드 스키마 맞춤)
  const fallbackSets = [
    {
      id: 'set1',
      day: '1일차: 감성 카페 투어 (예시)',
      memos: [
        { 
          id: 'm1', time: '10:00 AM', category: 'CAFE', // 👈 추가됨
          title: '📍 안목해변 카페거리', 
        ...
        { 
          id: 'm2', time: '12:30 PM', category: 'RESTAURANT', // 👈 추가됨
          title: '🍴 강릉 중앙시장 먹거리', 
    ...
    },
    {
      ...
      memos: [
        { 
          id: 'm3', time: '09:30 AM', category: 'SIGHTSEEING', // 👈 추가됨
          title: '🌊 경포호 산책', 
  ...
  // ✅ [수정 2] 카테고리에 따라 라벨과 색상을 반환하는 헬퍼 함수
  const getCategoryInfo = (category) => {
    switch (category) {
      case 'RESTAURANT':
        return { label: '🍚 식당', color: '#ff7e67', bg: '#fff0ec' };
      case 'CAFE':
        return { label: '☕ 카페', color: '#b08d55', bg: '#f7f3eb' };
      case 'SIGHTSEEING':
        return { label: '🎡 관광', color: '#6c5ce7', bg: '#f0f0ff' };
      default:
        return { label: '📍 장소', color: '#888', bg: '#f5f5f5' };
    }
  };
  ...
  const fetchAiPlan = async (extraMessage = "") => {
    ...
    try {
      const res = await api.post("/ai/plan", payload, { timeout: 60000 });
                            ...
                            <div className="memo-body">
                              <div className="memo-meta-row">
                                ...
                                <span className="category-badge" style={{
                                    ...
                                    fontWeight: 'bold',
                                    marginLeft: '8px',
                                    border: `1px solid ${catInfo.color}40` // 연한 테두리
                                }}>
                                  {catInfo.label}
                                </span>
                              </div>
